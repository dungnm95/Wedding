...
jQuery(document).ready(function() {
    "use strict";
    jQuery(".small-device-menu-link").pageslide({
        direction: "right",
        modal: true
    });
    jQuery(window).load(function() {
    ...
    jQuery('.collapse-btn').click(function() {
        if (jQuery('.collapse-btn i').hasClass('ion-chevron-left')) {
            jQuery('.gallery-info').fadeOut(500);
            jQuery('.single-gallery .entry-header').animate({
                marginLeft: '0px'
            }, 500, function() {
            ...
            jQuery('.collapse-btn i').removeClass('ion-chevron-left').addClass('ion-chevron-right');
            jQuery('.collapse-btn').animate({
                marginLeft: '-60px'
            }, 800, function() {});
        ...
        } else if (jQuery('.collapse-btn i').hasClass('ion-chevron-right')) {
            jQuery('.single-gallery .entry-header').animate({
                marginLeft: '300px'
            }, 500, function() {
                if (jQuery('.single-gallery.sly').length > 0) {
            ...
            jQuery('.collapse-btn i').removeClass('ion-chevron-right').addClass('ion-chevron-left');
            jQuery('.collapse-btn').animate({
                marginLeft: '0px'
            }, 800, function() {});
    ...
    jQuery(function() {
        jQuery('a.close').click(function() {
            ...
            jQuery.cookie(cookies_prefix + "_tooltip", 'closed', {
                expires: 365,
                path: '/'
            });
...
function viewPort() {
    ...
    for (var i = 0; i < wSizes.length; i++) {
        if (size >= wSizes[i]) {
            jQuery('.cosmo-comments .fb_iframe_widget iframe,.cosmo-comments .fb_iframe_widget span').css({
                'width': jQuery('.cosmo-comments.twelve.columns').width()
            });
            break;
        }
    }
...
jQuery(document).ready(function() {
    ...
    function cycleThru(variable, j) {
        ...
        jQuery(variable + " div:eq(" + j + ")").css('display', 'block').animate({
            opacity: 1
        }, 600).animate({
            opacity: 1
        }, delay).animate({
...
function searchAction(activeBoth) {
    ...
    if (jQuery('.search-btn').hasClass('searching')) {
        ...
        if (jQuery(window).width() < 768) {
            jQuery('body').animate({
                scrollTop: jQuery('#main>.main-container').offset().top - 30
            }, 'slow', 'swing', function() {
...
if (jQuery('body').hasClass('single-format-gallery')) {
    jQuery(window).load(function() {
        jQuery('.featimg.resized-img').animate({
            opacity: 1
        }, 500);
    });
}
...
jQuery('.header-collapser').click(function() {
    ...
    if (jQuery(this).hasClass('ion-chevron-up')) {
        jQuery(this).removeClass('ion-chevron-up').addClass('ion-chevron-down');
    } else {
        jQuery(this).removeClass('ion-chevron-down').addClass('ion-chevron-up');
    }
    setTimeout(function() {
        if (jQuery('.frame').length) {
            jQuery('.single-gallery #main.sly .main-container').not('.protected-gallery').height(getGalleryHeight() + 5);
    ...
    if (!jQuery('body').hasClass('single-gallery', 'sly') && is_mobile.logic && jQuery('.small-device-nav').hasClass('swipe')) {
        (function($) {
            ...
            mc.on("panright", function(ev) {
                ...
                $.pageslide({
                    href: $('.small-device-menu-link').attr('href')
                });
            });
        })(jQuery);
    }
